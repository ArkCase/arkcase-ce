...
if [ "$NEW_BIND_PWD" == "" ]; then
	echo "update-passwords.sh <new bind user password>"
	exit 1
fi
...
cp /data/arkcase-ce/vagrant/provisioning/stg-prc-facts.yml /data/arkcase-ce/vagrant/provisioning/stg-prc-facts.yml.`date +%Y-%m-%dT%H-%M`
